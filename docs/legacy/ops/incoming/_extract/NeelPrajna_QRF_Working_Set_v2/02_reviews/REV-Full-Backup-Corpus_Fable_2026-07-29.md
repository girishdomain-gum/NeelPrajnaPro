...
2. **F-11:** authorize the mockup-banner regeneration + corrections-log addendum (Architect executes, IVF-style render check before repackage).
3. Prior list, unchanged: Integration-Path architecture as the frozen basis; inverted plan (minimal Level-1 → ARCH-011/H-07 sprint → documents); Knowledge Publication Boundary as constitutional rule; role amendments; **window designation for the H-07 trade population** (permanently-human power).
