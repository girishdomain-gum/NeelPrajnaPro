...
## 2. The ECF in one panel (normative source: the docs\scientific_model\ master §5)
...
## 3. The MML in one panel (normative source: the docs\scientific_model\ master §6)
...
## 4. The architecture map in one panel (normative source: the docs\architecture\ master — md twin + docx)
...
## 5. Process quick card (normative source: the docs\execution_plan\ + docs\roles\ masters)
Sprint rhythm: ARCH → Developer (fresh session, worktree, DEVQs) → IVF (drill first) → HC → REV → Owner Go/No-Go → GO + retro → execution_plan master §0 rewrite. Ops channel: scripts in ops\, logs out, Architect reads logs. Findings format: claimed / true / species / standing rule — immediately, factually, against a name, including one's own.
