...
`mockups_corrected/qrf_research_console_mockup{,_v1.1,_v1.2}.html` in the
working-set package. These supersede the mangled part2.zip copies, which
...
## Second addendum, same day — the ARCH-011 numbering collision

**What was claimed/done:** the Architect issued the NeelPrajna integration
sprint instruction as "ARCH-011."

**What was true:** `GEN2_EXECUTION_ROADMAP.md` (in the repository, predating
that issuance) reserves ARCH-011 for Generation 2, Sprint 1.

**Species:** numbering collision (the F-12 species), committed by the same
Architect who had flagged F-12 for ADRs earlier the same day.

**Remedy (executed):** the sprint instruction renumbered **ARCH-NP-001**
with a renumbering note appended to the file; every reference across the
estate updated; Constitution v2.0 §5.4 extended to namespace ARCH numbers
per track (`ARCH-###` = QRF/Gen-2 line, `ARCH-NP-###` = integration line).
