...
**NP-D-010 · H-07 detector definition §5 v1.1 (NP-ADR-008, Owner-ratified 2026-07-30).** The H-07 324-trade population was produced by a **structurally different detector** than §5 v1.0 defines — M5 single-timeframe, fixed 30-tick pool tolerance, max/min pool level, and no REVERSAL_CONFIRMED/MSS stage at all. **§5 v1.0 stays frozen forever** as the faithful record of the MQL5 gate `T3_SweepFVGGate.mqh`; **v1.1 is not a correction of it but the first authoritative documentation of the definition the evidence embodies.** Three binding non-equivalence statements (Chief Scientist, adopted verbatim): v1.1 is not equivalent to v1.0 · it is a new hypothesis bound to the v1.1 lineage · judging the original T3/MSS detector requires a separate implementation and fresh out-of-sample evidence. Registrations use lineage `h007_np_liquidity_sweep_v1_1`, detector `neelprajna.liquidity_sweep@1.1.0`, family **`xauusd/neelprajna`** (identical across all 19, or the α-budget does not bind), scope `xauusd_m5_vantage`, cost model `xauusd_retail_h07` at **$0.41/oz** (measured spread + declared slippage/commission), family trial count **19** → per-claim bar **p < 0.00263**. E2 restated as the POOL→SWEEP arrangement claim under null design N2. Full text and evidence: `ops\NP-ADR-H07_definition_v1.1_draft_v2.0.md` (sealed as NP-ADR-008), `ops\CS_REVIEW_H07_v1.1_2026-07-30.md`, `ops\PRE_RATIFICATION_REVIEW_H07_v1.1.md`, `ops\POST_CORRECTION_VERIFICATION_H07_v1.1.md`. Journal: J-034.

**NP-D-011 · Execution-model parity precedes further evidence collection (Owner-ruled 2026-07-30, NP-S1 Go/No-Go D2).** The audited execution engine shall be extended to express **variable stop distance, variable targets and richer exit rules before any further R6 evidence is collected** (Execution Plan §6, WO-P). Basis: in NP-S1 the engine could not express H-007's evidenced per-trade stop/target, so the hypothesis was registered with `stop_offset: null, target_offset: null, exit_rule: time_stop` and the Battery judged *sweep-then-hold-12-bars* rather than the evidenced strategy (NOTE-NP-001). Every population judged before parity exists inherits that limitation, and every comparison built on it carries NP-S1's qualification permanently; collecting months of data first would multiply the defect across everything that data later judges. Origin: Chief Scientist REV of NP-S1 (APPROVED 8.8/10, GO with qualification).

**NP-D-012 · Specification completeness (standing rule, Owner-adopted 2026-07-30, NP-S1 Go/No-Go D3).** *Any normative specification defining a computation must be sufficient for an independent implementation to reproduce that computation's outputs without consulting the implementation.* Proposed independently by the Architect (NP-ADR-008 Appendix B.9) and by the Chief Scientist, after three instances surfaced within one sprint: the Battery's step count (documents said nine, `battery.py` and ARCH-006 §3 say eight); the H-007 detector definition (two faithful readers of NP-ADR-008 §3 produced 325 and 331 sweeps, a P6 failure closed by Appendix B); and ARCH-006's *"seeded bootstrap CI"*, which the IVF had to read `battery.py` to pin to 2,000 resamples and a specific RNG. Where a specification is insufficient, that is a **defect to be pinned, not a gap for the reader to fill**.

**NP-D-013 · No calendar-bound sprints (standing rule, Owner-adopted 2026-07-31, NP-S2 G1 seal).** *Every sprint must have a clear, measurable definition of "done" achievable without waiting for calendar time.* Where an activity inherently takes months (e.g. evidence collection), the act of starting and supervising it is one sprint's deliverable; the completed dataset is a different sprint's precondition — never one sprint spanning the whole calendar duration. Adopted alongside the ruling that narrowed NP-S2's scope to WO-P alone, deferring R6 collection to NP-S3 with its own preflight and G1. Rationale: long-running collection blurs sprint boundaries and raises concurrency questions (can a second sprint open while collection runs? which sprint owns shared infrastructure?) that do not need solving before they are real; software-building and evidence-collection are different kinds of work and merit separate sprints. Recorded in `ops\SPRINT_STATE_MACHINE_v1.1.md` §13. Journal: J-040.

## Change Record
- v1.3 (2026-07-31): NP-D-013 appended — no calendar-bound sprints; NP-S2 scope narrowed to WO-P.
- v1.2 (2026-07-30): NP-D-011 and NP-D-012 appended — execution-model parity before further evidence collection; the specification-completeness standing rule.
- v1.1 (2026-07-30): NP-D-010 appended — H-07 §5 v1.1 detector definition ratified as NP-ADR-008.
