...
| Instrument symbol (PINNED, O-023) | `XAUUSD` — EXACT string, on the Vantage feed, and nothing else. The same terminal also lists `XAUUSD.crp`; the Owner has ruled it is NEVER read, exported, ingested, or registered by this project. Enumerating it in a symbol inventory is fine; USING it is forbidden. The pin is exact-match, never a `XAUUSD*` prefix (a prefix would readmit the excluded symbol). Historical note: the retired NPSU estate's filenames carry a third string, `XAUUSDm`, from the old stack — sealed history, not a current source. |
...
| External evidence store (PINNED, AM-07/O-049) | `F:\NeelPrajnaProData\incoming` — raw bulk market data (exported bar/tick CSVs and comparable bulk evidence files) is NEVER git-tracked; this store sits outside the repo root so no future .gitignore-path mismatch can pull it back in (the near-miss this pin closes: `data/incoming/` vs the ignore rule's `ops/incoming/`, 23,066 of 24,584 diff lines being raw CSVs staged for main). Named constant: `EXTERNAL_EVIDENCE_STORE` in `ivf/mt5/export_xauusd_m5.py`. Provenance twins (small, the scientific record) stay git-tracked and now carry the CSV's own `csv_sha256`; `scripts/verify_csv_provenance.py` recomputes and refuses loudly on mismatch. `data/incoming/`'s two pre-AM-07 CSVs stay in dev's git history (no history rewrite) with their originals also left on disk; the directory itself is now in .gitignore alongside `ops/incoming/` (both named explicitly, never merged into one rule). Backup coverage for this store is OWNER-OWNED and UNCONFIRMED as of 2026-08-02 — do not assume either way. |
